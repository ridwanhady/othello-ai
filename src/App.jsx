...
const App = () => {
  ...
  const moveRandom = (arr = allowedMoves) => {
    moveTo(arr[Math.floor(Math.random() * arr.length)])
  }
  ...
  const moveTo = move => {
    ...
    fetch(API_URL + `move`, {
      ...
      .then(res => res.json())
      .then(data => {
        if (data.possible_move.length === 0 || isNullOrUndefined(board) ? (data.possible_move.length === 0) : (board.filter(b => b === 0)).length <= 1) {
          ...
          if (random) moveRandom(data.possible_move)
        }
      })
  }
  ...
  return (
    <div id="container">
      <div id="start-modal" className={started ? 'hide' : null}>
        <span>
          <label onWheel={toggleRandom} onClick={toggleRandom}>{random ? 'random' : 'be'}</label>
          <label onWheel={togglePlayer} onClick={togglePlayer}>
            {player ? <> <label style={{ color: 'white', textShadow: "-1px -1px 0 #aaa, 1px -1px 0 #aaa, -1px 1px 0 #aaa, 1px 1px 0 #aaa" }}>white</label></> : <> <label style={{ color: 'black' }}>black</label></>}
          </label>
        </span>
